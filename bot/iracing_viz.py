...
class iRacingVisualizer:
    ...
    def create_schedule_table(self, series_name: str, schedule: List[Dict], week_filter: str = "full") -> BytesIO:
        ...
        if week_filter == "current":
            ...
            # Show next week
            current_week = self._get_current_iracing_week(schedule)
            next_week = (current_week + 1) % len(schedule)
        ...
        row_height = 0.8
        chart_height = max(8, 3 + (num_weeks * row_height))
        ...
        total_height = num_weeks + 3
        ax.set_ylim(0, total_height)

        # Title
        title_y = total_height - 0.8
        ax.text(7, title_y, f"{series_name}",
               ha='center', fontsize=21, fontweight='bold', color='#ffffff')

        subtitle_y = total_height - 1.4
        ax.text(7, subtitle_y, title_suffix,
        ...
        header_text_y = headers_y - 0.05
        ax.text(1.5, header_text_y, "Week", fontsize=14, color='#60a5fa',
               fontweight='bold', ha='center', va='center')
        ax.text(5.5, header_text_y, "Track", fontsize=14, color='#60a5fa',
               fontweight='bold', ha='left', va='center')
        ...
        for idx, week_data in enumerate(filtered_schedule):
            ...
            ax.text(1.5, y_pos, f"Week {week_num + 1}",
                   fontsize=13, color='#60a5fa', va='center', ha='center', fontweight='bold')
            ...
            if len(full_track_name) > 60:
                full_track_name = full_track_name[:57] + '...'

            ax.text(5.5, y_pos, full_track_name,
                   fontsize=12, color='#ffffff', va='center', fontweight='bold')
    ...
    def _get_current_iracing_week(self, schedule: List[Dict]) -> int:
        ...
        # iRacing weeks typically start on Tuesday
        # Try to find the current week by looking at start dates if available
        now = datetime.datetime.now(datetime.timezone.utc)

        for week in schedule:
            # Check if week has start_date field
            start_date = week.get('start_date')
            if start_date:
                # Parse the date and check if we're in this week
                try:
                    week_start = datetime.datetime.fromisoformat(start_date.replace('Z', '+00:00'))
                    # Weeks are typically 7 days
